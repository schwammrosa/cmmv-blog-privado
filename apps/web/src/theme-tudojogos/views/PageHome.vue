                        ...
                        <div v-for="(post, index) in coverPosts.carousel" :key="post.id"
                            ...
                            <a :href="`/post/${post.slug}`" class="block h-full">
                                ...
                                <div class="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 via-black/70 to-transparent text-white">
                                    ...
                                    <span class="gamer-button inline-block px-4 py-2 text-sm font-medium rounded-md transition-all hover:shadow-lg text-white" style="color: white !important;">
                                        Continuar lendo
                                    </span>
                                </div>
                            </a>
                            ...
                            <a :href="`/post/${post.slug}`" class="block h-full">
                                <div class="relative h-full">
                                    ...
                                    <div class="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/90 via-black/70 to-transparent text-white">
                                        ...
                                        <span class="gamer-button inline-block px-3 py-1 text-sm font-medium rounded-md transition-all hover:shadow-lg text-white" style="color: white !important;">
                ...
                <div class="flex-grow">
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div class="lg:col-span-2">
                            ...
                            <!-- Elemento âncora para observar a rolagem -->
                            <div ref="maisConteudoObserver" class="h-4"></div>
                            
                            <div v-if="posts.length > (featuredPost ? 5 : 4) && mostrarMaisConteudo" class="mais-conteudo-section">
...
// Paginação para a seção Mais Conteúdo
const maisConteudoPage = ref(1);
const maisConteudoPerPage = ref(6);
const mostrarMaisConteudo = ref(false);
const maisConteudoObserver = ref<HTMLElement | null>(null);

// Determinar se estamos em modo de desenvolvimento (sem acessar process.env no template)
const isDevMode = ref(false);
onMounted(() => {
    // Verificar se estamos em modo de desenvolvimento no cliente apenas
    try {
...
const maisConteudoTotalPages = computed(() => {
    const totalItems = posts.value.length - (featuredPost ? 5 : 4);
...
const loadPosts = async () => {
    try {
        ...
        // Carregamos todos os posts de uma vez, com um limite generoso
        ...
        if (response) {
            ...
            // Resetar a página atual para a primeira
...
onMounted(async () => {
    ...
    // Configurando o Intersection Observer para carregar "Mais Conteúdo" sob demanda
    if (maisConteudoObserver.value && 'IntersectionObserver' in window) {
        const observer = new IntersectionObserver(
            (entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        console.log('%c[Lazy Loading] Elemento observado visível! Carregando seção "Mais Conteúdo"...', 'background: #4caf50; color: white; padding: 2px 5px; border-radius: 3px;');
                        mostrarMaisConteudo.value = true;
                        // Adicionar pequeno atraso para exibir o console.log de conclusão após a renderização
                        setTimeout(() => {
                            console.log('%c[Lazy Loading] Seção "Mais Conteúdo" carregada com sucesso!', 'background: #2196f3; color: white; padding: 2px 5px; border-radius: 3px;');
                        }, 100);
                        observer.disconnect(); // Desconecta o observer após carregar
                    }
                });
            },
            { 
                // Começar a carregar quando estiver a 200px antes de chegar na seção
        ...
        console.log('%c[Lazy Loading] Intersection Observer não suportado. Carregando "Mais Conteúdo" imediatamente.', 'background: #ff9800; color: white; padding: 2px 5px; border-radius: 3px;');
        mostrarMaisConteudo.value = true;
    }
});
