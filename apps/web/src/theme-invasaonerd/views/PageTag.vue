        ...
        <div class="lg:max-w-4xl md:max-w-3xl mx-auto">
            ...
            <div v-else class="bg-white rounded-lg p-6 article-container overflow-hidden">
                <header class="border-b border-neutral-200 pb-4 mb-6 pr-4 pt-4">
                    ...
                    <div class="text-sm text-neutral-500">{{ data.tag.postCount }} posts nesta tag</div>
                </header>
                ...
                <div v-else-if="posts.length > 0" class="space-y-10 post-content prose prose-sm sm:prose prose-neutral max-w-none">
                    <article v-for="post in posts" :key="post.id" class="border-b border-neutral-200 pb-8 last:border-0">
                        ...
                        <h2 class="text-2xl font-bold text-neutral-900 mb-3">
                            <a :href="`/post/${post.slug}`" class="hover:text-[#ff3e3e] transition-colors" aria-label="Ler mais sobre este post">
                                {{ post.title }}
                            </a>
                        </h2>
                        ...
                        <div class="mt-4">
                            ...
                            class="inline-flex items-center text-[#ff3e3e] font-medium hover:text-[#d12222] transition-colors">
                                Ler mais
...
hasMorePosts.value = posts.value.length < (data.value.tag?.postCount || 0);
...
const loadMorePosts = async () => {
    ...
    try {
        ...
        if (response && response.posts && response.posts.length > 0) {
            ...
            hasMorePosts.value = posts.value.length < (response.tag?.postCount || 0);
        } else {
