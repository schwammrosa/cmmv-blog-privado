<template>
    <section v-if="posts.length > 0" class="mb-8 md:block hidden">
        ...
        <div v-else-if="coverSettings.layoutType === 'split'" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
            <div class="md:col-span-2 bg-white rounded-lg overflow-hidden shadow-md">
                <a v-if="coverPosts.splitMain" :href="`/post/${coverPosts.splitMain.slug}`" class="block rounded-lg overflow-hidden shadow-md bg-white h-full">
                    <div class="relative w-full h-full min-h-[220px]" style="min-height:375px;">
                        <OptimizedImage
                        ...
                        <div class="absolute inset-0 flex flex-col justify-end p-4 bg-gradient-to-t from-black/90 via-black/70 to-transparent text-white">
                            ...
                            <h2 v-if="coverPosts.splitMain" class="text-xl font-bold mb-1 truncate">{{ coverPosts.splitMain.title }}</h2>
                            <p v-if="coverPosts.splitMain" class="text-base mb-1 truncate">{{ coverPosts.splitMain.excerpt }}</p>
            ...
            <div class="md:col-span-1 flex flex-col gap-4">
                <div v-for="(post) in coverPosts.splitSide" :key="post.id" class="flex-1 bg-white rounded-lg overflow-hidden shadow-md">
                    <a :href="`/post/${post.slug}`" class="block rounded-lg overflow-hidden shadow-md bg-white" style="height:180px;max-height:180px;">
                        <div class="relative w-full h-full" style="height:180px;">
                            ...
                            <div class="absolute inset-0 flex flex-col justify-end p-2 bg-gradient-to-t from-black/90 via-black/70 to-transparent text-white">
                                ...
                                <h3 class="text-sm font-bold mb-1 truncate">{{ post.title }}</h3>
